...
const config = require("../../Akash.json");
...
const logger = require("./akashc.js");
...
global.utils = require("./akashd.js");
global.loading = require("./akashc.js");
...
var ryukoValue;
try {
  global.client.ryukoPath = join(global.client.mainPath, "../configs/Akash.json");
...
var configValue;
try {
  global.client.configPath = join(global.client.mainPath, "../../Akash.json");
  configValue = require(global.client.configPath);
  logger.loader(`deploying ${chalk.blueBright('AKASH')} file`);
} catch (e) {
  return logger.loader(`cant read ${chalk.blueBright('AKASH')} file`, "error");
}
try {
  for (const key in configValue) global.config[key] = configValue[key];
  logger.loader(`deployed ${chalk.blueBright('AKASH')} file`);
} catch (e) {
...
try {
  if (!global.config.BOTNAME) {
    logger.error(`please enter your bot name in ${chalk.blueBright('Akash.json')} file`);
    process.exit(0);
  }
...
try {
  var appStateFile = resolve(join(global.client.mainPath, "../../Akashstate.json"));
  ...
  logger.loader(`deployed ${chalk.blueBright('Akashstate')} file`)
} catch (e) {
  return logger.error(`can't read ${chalk.blueBright('Akashstate')} file`)
}
  ...
  console.log(chalk[`${cnslEvent.logger.colors.cmdStarter}`](`${cnslEvent.logger.strings.cmdStarter}`));
        for (const command of listCommand) {
          try {
            ...
            if (!config?.category) {
              try {
                throw new Error(` • HEY AKASH •  ${command} category is not in the correct format or empty`);
              } catch (error) {
                console.log(chalk.red(error.message));
                continue;
              }
            }
            const configures = require(`../../Akash.json`);
            if (configures.premium) {
              if (!config?.hasOwnProperty('premium')) {
                console.log(`• HEY AKASH • `, chalk.hex("#ff0000")(command) + ` does not have the "premium" property.`);
                continue;
              }
            }
            if (!config?.hasOwnProperty('prefix')) {
              console.log(`• HEY AKASH • `, chalk.hex("#ff0000")(command) + ` does not have the "prefix" property.`);
              continue;
            }

            if (global.client.commands.has(config.name || '')) {
              console.log(chalk.red(`• HEY AKASH •  ${chalk.hex("#FFFF00")(command)} module is already deployed.`));
              continue;
            }
            ...
            if (envConfig) {
              ...
              var ryukoPath = require('../configs/Akash.json');
        ...
        for (const ev of events) {
          try {
            ...
            if (!config || !config.name || !run) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#FFFF00")(ev)} module is not in the correct format. `, "• AKASH EVENT •");
              continue;
            }
            ...
            if (global.client.events.has(config.name)) {
              global.loading.err(`${chalk.hex('#ff7100')(``)} ${chalk.hex("#FFFF00")(ev)} module is already deployed.`, "• AKASH EVENT •");
              continue;
            }
            ...
            global.loading(`${crayon(``)}successfully deployed ${chalk.blueBright(config.name)}`, "• AKASH EVENT •");
          }
          catch (err) {
    ...
    global.custom = require('../../Akash.js')({ api: loginApiData });
