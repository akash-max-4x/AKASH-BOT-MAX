...
module.exports.onLoad = function () {
	...
	const path = join(__dirname, "Akash", "joinGif");
	if (existsSync(path)) mkdirSync(path, { recursive: true });	

	const path2 = join(__dirname, "Akash", "joinGif", "randomgif");
...
module.exports.run = async function({ api, event }) {
	...
	if (event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
		...
		const fs = require("fs");
		return api.sendMessage("AKASH BOT VERSION MAX", event.threadID, () => api.sendMessage({body:`________আসসালামু আলাইকুম___________
...
◑কিন্তু আমাকে কন্ট্রোল করার জন্য প্রয়োজন আমার বস আকাশ বস কে🤙
...
◑ ফেসবুক লিংক ☞ https://www.facebook.com/akash.black.hacker.bd


🐰\n\nUse ${global.config.PREFIX}help to see commands.\n\nexample :\n${global.config.PREFIX}mark (text)\n${global.config.PREFIX}lexi (text)\n${global.config.PREFIX}trump (text)\n${global.config.PREFIX}info 
`, attachment: fs.createReadStream(__dirname + "/Akash/joinmp4/akash.mp4")} ,threadID));
	}
	else {
		try {
			...
			const randomPath = readdirSync(join(__dirname, "Akash", "joinGif", "randomgif"));

			if (existsSync(pathGif)) formPush = { body: msg, attachment: createReadStream(pathGif), mentions }
			else if (randomPath.length != 0) {
				const pathRandom = join(__dirname, "Akash", "joinGif", "randomgif", `${randomPath[Math.floor(Math.random() * randomPath.length)]}`);
